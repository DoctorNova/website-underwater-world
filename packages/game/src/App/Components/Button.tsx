...
export const buttonStyles = {
  ...
  hover: {
    default: "bg-white/10 backdrop-blur-md border-white/30 text-white",
    destructive: "bg-destructive/90",
    outline: "bg-white/10 backdrop-blur-md border-white/30",
    secondary: "bg-primary border-primary text-primary-foreground",
...
export function Button({ className, variant, size, hover, ...props }: ComponentProps<"button"> & { className?: string, variant?: Variants, size?: Size, hover?: boolean}) {
  const v = variant ?? "default";
  return (
    <button
      className={cn(buttonStyles.always, buttonStyles.variant[v], buttonStyles.size[size ?? "default"], hover && buttonStyles.hover[v], className)}
      {...props}
    />
  );
}
