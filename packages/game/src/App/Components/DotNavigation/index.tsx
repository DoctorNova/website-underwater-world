...
export function DotNavigation({
  ...
  for (let index = 0; index < count; index++) {
    ...
    listOfDots.push(
      <div key={index} className="relative w-6 h-6 flex items-center justify-center cursor-pointer group rounded-md hover:bg-white/10 hover:backdrop-blur-md hover:border hover:border-white/30" onClick={() => onClick?.(index)}>
        {/* Outer pulsing ring */}
        {isActive && (
          <div
            className="absolute w-full h-full rounded-full bg-orange-500/30 animate-ping"
        ...
        {isActive && (
          <svg
            ...
            className={cn("absolute w-full h-full -rotate-90", !isActive && "hidden")}
            viewBox="0 0 32 32"
            onAnimationEnd={() => {
